...
class EDD_Theme_Updater_Admin {
	...
	/**
	 * EDD_Theme_Updater_Admin constructor. Initialize the class.
	 *
	...
	public function __construct( $config = array(), $strings = array() ) {
		...
		add_filter( 'http_request_args', array( $this, 'disable_wporg_request' ), 5, 2 ); //phpcs:ignore WordPressVIPMinimum.Hooks.RestrictedHooks.http_request_args
	}
	...
	public function license_menu() {
		...
		add_submenu_page(
			$rtcamp_slider->slider_options_name,
		...
		<div class="wrap rtp-admin-page">
			...
			<form method="post" action="options.php">
				...
				<table class="form-table">
					<tbody>
						<tr valign="top">
							<th scope="row" valign="top"><?php echo esc_html( $strings['license-key'] ); ?></th>
							<td>
					...
					if ( $license ) {
						?>
						<tr valign="top">
							<th scope="row" valign="top"><?php echo esc_html( $strings['license-action'] ); ?></th>
							<td>
								<?php
	...
	public function get_api_response( $api_params ) {
		...
		if ( is_wp_error( $response ) ) {
			wp_die( esc_html( $response->get_error_message() ), esc_html__( 'Error', 'foodmania' ) . esc_html( $response->get_error_code() ) );
		}
	...
	public function activate_license() {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			if ( is_wp_error( $response ) ) {
				...
				$message = __( 'An error occurred, please try again.', 'foodmania' );
			}
		} else {
				...
				switch ( $license_data->error ) {
					case 'expired':
						$message = sprintf(
							...
							esc_html__( 'Your license key expired on %s.', 'foodmania' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_datetime( 'timestamp' ) ) ) 
						);
						break;

					case 'revoked':
						$message = __( 'Your license key has been disabled.', 'foodmania' );
						break;

					case 'missing':
						$message = __( 'Invalid license.', 'foodmania' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = __( 'Your license is not active for this URL.', 'foodmania' );
						break;

					case 'item_name_mismatch':
						/* translators: %s is user name */
						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'foodmania' ), 'name' );
						break;

					case 'no_activations_left':
						$message = __( 'Your license key has reached its activation limit.', 'foodmania' );
						break;
				...
				if ( ! empty( $message ) ) {
					...
					$redirect = add_query_arg(
						array(
							...
							'rt_nonce'            => wp_create_nonce( $this->theme_slug . '_license_key_nonce' ),
						),
						$base_url
					);
		...
		wp_safe_redirect( admin_url( 'admin.php?page=' . $this->theme_slug . '-license' ) );
		exit();
	...
	public function check_license() {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( isset( $license_data->expires ) && 'lifetime' !== $license_data->expires ) {
				$expires    = date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_datetime( 'timestamp' ) ) ); 
 ...
 */
function edd_foodmania_theme_admin_notices() {

	if ( isset( $_GET['sl_theme_activation'] ) && ! empty( $_GET['message'] ) && ! empty( $_GET['rt_nonce'] ) && ! empty( $_GET['theme'] ) ) {

		$valid_nonce = wp_verify_nonce( sanitize_key( $_GET['rt_nonce'] ), sanitize_key( $_GET['theme'] ) . '_license_key_nonce' );

		if ( ! $valid_nonce ) {
			wp_nonce_ays( __( 'activate theme', 'foodmania' ) );
		}
...
add_action( 'admin_notices', 'edd_foodmania_theme_admin_notices' );
